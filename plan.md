...
- [ ] Set up PySpark Structured Streaming to consume from Kafka (`rider-location` topic).
- [ ] Implement Uber H3 Spatial Indexing mapping lat/lng coordinates to H3 Hexagons.
...
- [ ] Generate/Extract a training dataset from historical data.
- [ ] Train offline ML models for ETA Prediction (Regression) and Fraud/Ghost Rider Detection (Isolation Forest).
- [ ] Deploy ML models directly inside the PySpark streaming pipeline for real-time inference.
- [ ] Emit inferences to a new `rider-predictions` Kafka topic.
